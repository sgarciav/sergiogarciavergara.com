...
document.addEventListener("DOMContentLoaded", () => {
  const parent = document.querySelector('.wrapper'); // Assuming parent container exists in the HTML
  ...
  // Set TOC width based on longest item
  let maxWidth = 0;
  tocList.childNodes.forEach(item => {
    const itemWidth = item.offsetWidth;
    if (itemWidth > maxWidth) {
      maxWidth = itemWidth;
    }
  });
  tocList.style.width = `${maxWidth + 30}px`;
